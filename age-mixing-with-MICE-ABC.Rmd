...
MICE_ABC.incremental <- function(targets = targets.master + 0.3 * targets.diff,
  ...
  previous.data <- do.call(rbind, unlist(previous.data.lists[data.elements.location], recursive = FALSE))b # The input and output of ALL the simulations run until now.
  ...
  # pre.previous.calib.uls <- as.numeric(apply(pre.experiments, 2, max))
    ...
    for(i in 1:length(targets)) {
      nam <- paste0("y.", i, ".sq.rel.dist")
      val <- ((previous.data[,i] - targets[i]) / targets[i])^2
      assign(nam, val)
      sum.sq.rel.dist <- sum.sq.rel.dist + get(nam)
    }
