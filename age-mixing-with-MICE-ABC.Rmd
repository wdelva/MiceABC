...
Important question: should we only have “general population” targets, or also hiv-status and gender specific targets? For now, only general population.
 
#### Sexual activity:
5. Degree distribution of new sex partners in the last year.
7. (if GAMM or other multi-parameter model): Extract age effect: Fitting NB distribution to data on number of new relationships in last year, with age covariate.
```{r nb.params}
library(dplyr)
library(MASS)
library(splines)
...
intercept.Bm <- glm.nb(totalpartnerslstyr~1,
                       ...
                       init.theta = 0.5)
summary(intercept.Bm)
shape.nb <- as.numeric(intercept.Bm$coefficients) #(r)
...
finalBm<-glm.nb(totalpartnerslstyr~ns(age, 3),
                data=md.wideBM,
                control=glm.control(maxit=100000, trace = 3),
                init.theta=0.5)
...
##############################################################################
madeforBM<-data.frame(age = 15:49, gender = "man", race = "African")
...
plot(seq(15, 49), tryBM, type="l", col="navyblue",main="Black men",xlab="Age",ylab="Partnerslastyear")
...
10. Concurrency: see Delva-Meng-Beauclair paper

11. Epidemic trajectory: prevalence by age and gender, by age-groups as per Beauclair 2017 PLoS ONE
