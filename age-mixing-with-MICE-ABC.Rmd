...
MICE_ABC.incremental <- function(targets.empirical = targets.empirical,
  ...
  while (wave <= maxwaves & intermediate.features != targets.empirical){
    ...
    for (steps.intermediate.targets in 0:100){ # We make 1% steps from empirical targets to experimental median features
      candidate.intermediate.features <- targets.empirical - (targets.diff * steps.intermediate.targets) / 100
      for (steps.RMSD.tol in 0:100){ # We make 1% steps from RMSD.tol.max to RMSD.tol = 0
        RMSD.tol <- RMSD.tol.max * (100 - steps.RMSD.tol) / 100
        ...
        if (large.enough.training.df == TRUE){
          if (RMSD.tol < candidate.RMSD.tol){
            candidate.RMSD.tol <- RMSD.tol
            calibration.list$intermediate.features[[wave]] <- candidate.intermediate.features
            calibration.list$steps.intermediate.targets[[wave]] <- steps.intermediate.targets # 0 means targets.empirical; 100 means experim.median.features
    ...
    intermediate.features <- calibration.list$intermediate.features[[wave]]
    RMSD.tol <- calibration.list$RMSD.tol[[wave]]
    
    # 4. Calculate RMSD values for these intermediate.features and the RMSD.tol found
    for (i in 1:length(intermediate.features)) {
      name.dist <- paste0("y.", i, ".sq.rel.dist")
      value.dist <- ((sim.results.with.design.df[,i + x.offset] - intermediate.features[i]) / intermediate.features[i])^2
      assign(name.dist, value.dist)
      sum.sq.rel.dist <- sum.sq.rel.dist + get(name.dist)
    }
    RMSD <- sqrt(sum.sq.rel.dist / length(intermediate.features))
