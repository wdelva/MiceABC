...
master.data <- agemix.simulate(param.set = c(8, 0.2), no.repeated.sim=5)
...
all.data.choice.index <- ifelse(nrow(all.data.selected) > nrow(all.data.filtered),
                                1,
                                2)
all.data.final <- all.data.choices[[all.data.choice.index]]
calibration.list$estimates <- all.data.final

summary(all.data.final)
pairs(all.data.final[,6:7])
dim(all.data.final[,6:7])

save.image(file = "/Users/delvaw/Google Drive/IBM course/tutorials/AgeMixing/Calibration/Thursday.evening.RData")
